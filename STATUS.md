...
- **Found a real conversion-killer bug while verifying endpoints:** the website's own "Integrate in 30 Seconds" code snippet advertises `POST /scrub` on pii-scrubber — **that endpoint doesn't exist, returns 404.** The real, correctly-implemented, x402-spec-compliant endpoint is `POST /mcp` (MCP/JSON-RPC body + `X-Payment-Tx` header) — confirmed working perfectly. So any human developer copy-pasting the site's own example would immediately hit a dead end and likely bounce. Backend is fine; only the marketing copy is wrong. **Not yet fixed — recommended next step.**
- **Next / open:**
  1. **Fix the broken `/scrub` code snippet on the site** — replace with the real working `POST /mcp` example (correctly documented in each service's own `/openapi.json`)
