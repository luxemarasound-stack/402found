...
function getConfig() {
  return {
    ...
    paymentAmount: process.env.PAYMENT_AMOUNT ?? "0.02",
...
export function getPaymentRequirement(): PaymentRequirement {
  ...
  return {
    ...
    description: "Prevents runaway cloud costs from AI agent usage. Enforces hard budget limits with configurable actions (throttle, pause, alert, kill).",
    ...
    outputSchema: {
      input: {
        ...
        properties: {
          ...
          currentSpend: { type: "number", description: "Amount to add to spend tracking" },
