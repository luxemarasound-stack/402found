...
export function getPaymentRequirement(): PaymentRequirement {
  ...
  return {
    ...
    resource: "https://card-registry.fly.dev/mcp",
    ...
    outputSchema: {
      input: {
        ...
        method: "POST",
        properties: {
          slug: { type: "string", description: "URL-safe identifier (lowercase, hyphens, 3-64 chars)" },
          card: { type: "object", description: "The agent-card.json object to host" },
        },
        required: ["slug", "card"],
      },
      output: {
        type: "object",
        properties: {
          hosted_at: { type: "string", description: "Public URL where the card is hosted" },
          slug: { type: "string" },
          status: { type: "string", enum: ["created", "updated"] },
        },
        required: ["hosted_at", "slug", "status"],
      },
    },
  };
}
