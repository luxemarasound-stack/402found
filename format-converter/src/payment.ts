...
export function getPaymentRequirement(): PaymentRequirement {
  ...
  return {
    ...
    resource: "https://format-converter.fly.dev/mcp",
    description: "Converts data between JSON, CSV, XML, YAML, Markdown, HTML, and TOML formats",
    ...
    outputSchema: {
      input: {
        ...
        properties: {
          data: { type: "string" },
          from: { type: "string", enum: ["json", "csv", "xml", "yaml", "markdown", "html", "toml"] },
          to: { type: "string", enum: ["json", "csv", "xml", "yaml", "markdown", "html", "toml"] },
        },
