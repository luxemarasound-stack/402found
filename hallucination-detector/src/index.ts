...
import { z } from "zod";
import { detectHallucinations } from "./detector.js";
...
mcpServer.tool(
  ...
  "Scores AI-generated text for likely hallucinated facts. Detects fabricated precision, invented citations, temporal impossibilities, hedged authority, entity confusion, and overconfident absolutes. Returns a 0-1 risk score, verdict, and detailed signals.",
  { text: z.string().describe("The AI-generated text to analyze for hallucination signals") },
  async ({ text }) => {
...
app.get("/openapi.json", (_req, res) => {
  res.json({
    ...
    info: {
      ...
      version: "1.0.0",
      description: "Scores AI output for likely hallucinated facts. Pay-per-request via x402 (USDC on Base).",
      guidance: "POST to /mcp with a JSON-RPC body calling 'detect_hallucinations' with a 'text' argument containing AI-generated text. Include X-Payment-Tx header. Returns { score (0-1), verdict, signals, sentence_count, flagged_count }.",
    },
    paths: {
      "/mcp": {
        post: {
          summary: "Score text for hallucination risk",
          description: "MCP endpoint for detect_hallucinations tool. Requires x402 payment (0.003 USDC on Base).",
          ...
          requestBody: { required: true, content: { "application/json": { schema: { type: "object", required: ["jsonrpc", "method", "params", "id"], properties: { jsonrpc: { type: "string", enum: ["2.0"] }, method: { type: "string", enum: ["tools/call"] }, params: { type: "object", required: ["name", "arguments"], properties: { name: { type: "string", enum: ["detect_hallucinations"] }, arguments: { type: "object", required: ["text"], properties: { text: { type: "string", description: "AI-generated text to analyze for hallucination signals" } } } } }, id: { type: "number" } } } } } },
          responses: { "200": { description: "Hallucination analysis result", content: { "application/json": { schema: { type: "object", properties: { score: { type: "number" }, verdict: { type: "string", enum: ["low", "medium", "high"] }, signals: { type: "array", items: { type: "object", properties: { type: { type: "string" }, text: { type: "string" }, reason: { type: "string" } } } }, sentence_count: { type: "integer" }, flagged_count: { type: "integer" } } } } } }, "402": { description: "Payment Required" } },
        },
      },
    },
  });
...
<title>Hallucination Detector — 402Found.dev</title>
<style>
...
<p>Scores AI output for likely hallucinated facts. Pay-per-request via x402 (USDC on Base).</p>
<div class="links">
...
app.listen(PORT, "0.0.0.0", () => {
  console.log(`Hallucination Detector MCP server running on port ${PORT}`);
