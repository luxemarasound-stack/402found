...
function getConfig() {
  return {
    ...
    paymentAmount: process.env.PAYMENT_AMOUNT ?? "0.10",
...
export async function verifyPayment(txHash: string): Promise<boolean> {
  ...
  if (!walletAddress) { console.error("WALLET_ADDRESS not configured"); return false; }
  ...
  const validLog = receipt.logs?.some((log: any) => {
    ...
    return ("0x" + log.topics[2].slice(26)).toLowerCase() === walletAddress.toLowerCase();
  });
  ...
  if (!blockData.result) return false;
  if (Date.now() - parseInt(blockData.result.timestamp, 16) * 1000 > MAX_TX_AGE_MS) return false;
  return true;
}
...
export function getPaymentRequirement(): PaymentRequirement {
  ...
  return {
    ...
    description: "Captures and monitors AI agent output quality over time. Baseline snapshots and drift detection.",
    mimeType: "application/json",
    maxTimeoutSeconds: 30,
    outputSchema: { input: { type: "http", method: "POST" }, output: { type: "object" } },
  };
}
